...
func TestAddBlog(t *testing.T) {
	Convey("Add blog", t, func() {
		...
		blog3 := models.Blog{
			/* ID:          GenerateUUID(8), */
			Title:       "blog3",
			Description: "blog3 deneme",
		}

		Convey("when the post request send", func() {

			reqBody, err := json.Marshal(blog3)

			req, _ := http.NewRequest(http.MethodPost, "/blogs", bytes.NewReader(reqBody))
			req.Header.Add("Content-Type", "application/json")
			...
			Convey("Then Added blog should return", func() {
				actualResult, err := repository.GetBlog(blog3.ID)

				So(err, ShouldBeNil)
				/* So(actualResult, ShouldHaveLength, 1) */
				So(actualResult, ShouldNotBeNil)
				/* So(actualResult.ID, ShouldEqual, blog3.ID) */
				So(actualResult.Title, ShouldEqual, blog3.Title)
				So(actualResult.Description, ShouldEqual, blog3.Description)
